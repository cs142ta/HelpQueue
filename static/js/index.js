...
function sortFilterData(startTime, endTime, allDays) {
    ...
    for (index in extendedStatsData) {
        if (typeof (extendedStatsData[index]) !== 'function') {
            if ((parseInt(extendedStatsData[index].dequeueTime) >= startTime && parseInt(extendedStatsData[index].dequeueTime) <= (endTime + ONE_HOUR_IN_MILISECONDS)) || allDays) {
...
function verifyStudentInputToggleButton() {
    ...
    if (queueActive) {
        ...
        } else //not valid input
        {
            ...
            $("#getHelpButton").html('Enter a question');
        }
        ...
        $("#questionInputLengthLeft").html(200 - theQuestion.length + " characters remaining");
    } else //queue is not active
    {
        $("#getHelpButton").addClass("btn-warning");
        $("#getHelpButton").html('The Help Queue is not currently active');
                ...
                if (!isNaN(parseInt(value))) {
                    ...
                    if (firstTime) {
                        ...
                        if (splitOnDaysOnly) {
                            ...
                            valueToPrint = valueToPrint == 0 ? 12 : valueToPrint;
                            //mod 12 gives 0. thats not what we want for time of day
	...
	*/
    ...
    for (var key in sortedData) {
        if (daySplitting) //keep things in terms of days
        {
            ...
            returnedValues = averageAndMedianValues(key, oneDaysHourStats, averageWaitTimes, medianGraphValues, numberHelpCountPerHour, numberPassOffCountPerHour, averageTimeWithTA);
        ...
        } else //split each day into hours
        {
            ...
            for (var lablesKey = 0; lablesKey < labels.length; lablesKey++) {
                ...
                if (!oneDaysHourStats.hasOwnProperty(labels[lablesKey]))
                    oneDaysHourStats[labels[lablesKey]] = [{
                        dequeueTime: 0,
                        enqueueTime: 0
                    }];
            }
            ...
            for (var lablesKey = 0; lablesKey < labels.length; lablesKey++) {
                returnedValues = averageAndMedianValues(labels[lablesKey], oneDaysHourStats, averageWaitTimes, medianGraphValues, numberHelpCountPerHour, numberPassOffCountPerHour, averageTimeWithTA);
    ...
    function averageAndMedianValues(index, oneTimeUnitStats, averageWaitTimes, medianGraphValues, numberHelpCountPerHour, numberPassOffCountPerHour, timeWithTAAvg) {
        ...
        if (isNaN(medianVal))
            //if there is no activty for a day this takes care of that
            medianVal = 0;
    ...
    return {
        'rawDataFilteredByHour': rawDataFilteredByHour,
        'daySplitting': daySplitting
    };
}
...
function extendedStatsGraphsButton(event) {
    ...
    if (startDate.length > 0 && startDate !== "" && startTime.length > 0 && startTime !== "" && endDate.length > 0 && endDate !== "" && endTime.length > 0 && endTime !== "") {
...
function countUpExtendedStatsForTables(sortedData, tallyPassOff) {
    ...
    for (var index = 0; index < sortedData.length; index++) //globalStudentNames
    {
        ...
        if (sortedData[index].passOff == tallyPassOff || sortedData[index].passOff == null || tallyPassOff == "all") {
            ...
            if (!taStudentData.hasOwnProperty([globalStudentNames[sortedData[index].netId]])) {
                ...
                taStudentData[globalStudentNames[sortedData[index].netId]] = temp;
                // convert netId to student name
            }

            //incremenet the jank
            if (sortedData[index].removedBy == sortedData[index].netId) {
                taStudentData[globalStudentNames[sortedData[index].netId]]['Themselves']++;
                //convert netID to student name
                taTotals['Themselves']++;
            } else {
                taStudentData[globalStudentNames[sortedData[index].netId]][taNetIdToName[sortedData[index].removedBy]]++;
                //first one is student, second is TA
                taTotals[convertNetIdToName(sortedData[index].removedBy)]++;
                //convert to TA name
            }
        }
    }
    ...
    return {
        'tableFormatedData': tableFormatedData,
...
function extendedStatsTableButton(event) {
    ...
    if ((startDate.length > 0 && startDate !== "" && startTime.length > 0 && startTime !== "" && endDate.length > 0 && endDate !== "" && endTime.length > 0 && endTime !== "") || allDays) {
        if (event.data.crunchNumbers) {
            ...
            //in case we want all days we got to check for this
...
function extendedStatsQuestionsButton(event) {
    ...
    if ((startDate.length > 0 && startDate !== "" && endDate.length > 0 && endDate !== "") || allDays) {
        if (event.data.crunchNumbers) {
        ...
        Notification.requestPermission(function(permission) {
            if (permission === 'granted') {
                var notification = new Notification(msg,{
                    body: 'A TA has something to tell you!',
                    tag: "MyNotify"
                });
...
//------------------------------------------------------------------------------------------------------
$(function() {
    //set stuff up once jquery loads
    ...
    getExtendedStatsToStart();
    //all this REALLY does will get names for the typeahead on the profile page.
    ...
    $('#extendedStatsDateInput .time').on('changeTime', {
        crunchNumbers: false
    }, extendedStatsGraphsButton);
    $('#extendedStatsDateInput .date').on('changeTime', {
        crunchNumbers: false
    }, extendedStatsGraphsButton);

    $('#extendedStatsStartDate').on('input', {
        crunchNumbers: false
    }, extendedStatsGraphsButton);
    $('#extendedStatsStartTime').on('input', {
        crunchNumbers: false
    }, extendedStatsGraphsButton);
    $('#extendedStatsEndDate').on('input', {
        crunchNumbers: false
    }, extendedStatsGraphsButton);
    $('#extendedStatsEndTime').on('input', {
        crunchNumbers: false
    }, extendedStatsGraphsButton);
    ...
    $("#updateExtendedStatsViewsTable").on("click", {
        crunchNumbers: true
    }, extendedStatsTableButton);
    ...
    $('#extendedStatsDateInputTable .time').on('changeTime', {
        crunchNumbers: false
    }, extendedStatsTableButton);
    $('#extendedStatsDateInputTable .date').on('changeDate', {
        crunchNumbers: false
    }, extendedStatsTableButton);

    $('#extendedStatsStartDateTable').on('input', {
        crunchNumbers: false
    }, extendedStatsTableButton);
    $('#extendedStatsStartTimeTable').on('input', {
        crunchNumbers: false
    }, extendedStatsTableButton);
    $('#extendedStatsEndDateTable').on('input', {
        crunchNumbers: false
    }, extendedStatsTableButton);
    $('#extendedStatsEndTimeTable').on('input', {
        crunchNumbers: false
    }, extendedStatsTableButton);
    ...
    $("#studentTabAllDataCheckbox").on("click", {
        crunchNumbers: false
    }, extendedStatsTableButton);

    //-------------------------------- Question tab ----------------------------------------------
    $("#updateExtendedStatsViewsQuestionsTable").on("click", {
        crunchNumbers: true
    }, extendedStatsQuestionsButton);
    ...
    //set up the listeners
    $('#extendedStatsDateInputQuestions .date').on('changeDate', {
        crunchNumbers: false
    }, extendedStatsQuestionsButton);

    $('#extendedStatsStartDateQuestionsTab').on('input', {
        crunchNumbers: false
    }, extendedStatsQuestionsButton);
    $('#extendedStatsEndDateQuestionsTab').on('input', {
        crunchNumbers: false
    }, extendedStatsQuestionsButton);
    ...
    $("#questionsTabAllDataCheckbox").on("click", {
        crunchNumbers: false
    }, extendedStatsQuestionsButton);
    ...
    function profileSearch(incomingData) {
        ...
        var questionHTML = "<div class='row'>" + "<div class='col-xs-6'>" + "<strong>Question</strong>" + "</div>" + "<div class='col-xs-2'>" + "<strong>Removed By</strong>" + "</div>" + "<div class='col-xs-2'>" + "<strong>Removed At</strong>" + "</div>" + "<div class='col-xs-2'>" + "<strong>Time spent with TA</strong>" + "</div>" + "</div>";
        ...
        for (index in incomingData) {
            if (globalStudentNames[incomingData[index].netId] == input) {
                ...
                if (!(incomingData[index].removedBy in taHelpCount)) {
                    taHelpCount[incomingData[index].removedBy] = {
                        'passOff': 0,
                        'help': 0
                    };
                }
                ...
                if (incomingData[index].passOff == 'true') {
                    ...
                    var questionHTML = "<div class='row'>" + "<div class='col-xs-6'>" + incomingData[index].question + "</div>" + "<div class='col-xs-2'>" + (incomingData[index].removedBy == incomingData[index].netId ? 'Themselves' : convertNetIdToName(incomingData[index].removedBy)) + "</div>" + "<div class='col-xs-2'>" + formatOutputDateAMPM(new Date(parseInt(incomingData[index].dequeueTime)), true, true) + "</div>" + "<div class='col-xs-2'>" + timeSpentOutCol + "</div>" + "</div>";
        ...
        var output = "<div class='row'>" + "<div class='col-xs-4'>" + "<strong>Name</strong>" + "</div>" + "<div class='col-xs-3'>" + "<strong>Help Count</strong>" + "</div>" + "<div class='col-xs-3'>" + "<strong>Pass Off</strong>" + "</div>" + "</div>";
        ...
        for (var i = 0; i < orderedTas.length; i++) {
            ...
            var output = "<div class='row'>" + "<div class='col-xs-4'>" + (studentName == input ? 'Themselves' : studentName) + "</div>" + "<div class='col-xs-3'>" + taHelpCount[ta].help + "</div>" + "<div class='col-xs-3'>" + taHelpCount[ta].passOff + "</div>" + "</div>";
            $("#profileTAHelpCount").append(output);
        }
    }
    ...
    $('#editRawDataDateInput .time').on('changeTime', {
        crunchNumbers: false
    }, setUpEditRawData);
    $('#editRawDataDateInput .date').on('changeTime', {
        crunchNumbers: false
    }, setUpEditRawData);

    $('#editRawDataStartDate').on('input', {
        crunchNumbers: false
    }, setUpEditRawData);
    $('#editRawDataStartTime').on('input', {
        crunchNumbers: false
    }, setUpEditRawData);
    $('#editRawDataEndTime').on('input', {
        crunchNumbers: false
    }, setUpEditRawData);
    ...
    $("#editRawDataGetDataBtn").on("click", {
        crunchNumbers: true
    }, setUpEditRawData);

    $('[data-toggle="tooltip"]').tooltip();
    verifyStudentInputToggleButton();
    //initialy disable the button
    $("#updateExtendedStatsViews").on("click", {
        crunchNumbers: true
    }, extendedStatsGraphsButton);
    ...
    $("#easterEggTrigger").on("click", function() {
        var output = '<div><center>' + '<object type="text/html" data="' + window.location.origin + addressBase + '/static/t-rex-runner-gh-pages/"' + ' width="600px" height="200px" style="overflow:auto;"></object>' + ' <br/>This help queue was designed and built by Blaine McGary during the Winter Semester 2016. It was ' + ' first used during the Winter 2016 semester in the class CS 240.' + '<br/>This CS 142 version is currently maintained by Gandalf the Grey.' + '</center></div>';
    ...
    $("#queueActive").on('change', function() {
        var value = {
            key: "queueActive",
            value: $("#queueActive").prop('checked')
        };
    ...
    $("#changeQuestionsPerDayBtn").on("click", function() {
        $("#changeQuestionPerDayModal").modal();
    });

    $("#changeQuestionPerDaySubmit").on("click", function() {
        var value = $("#changeQuestionPerDayInput").val();
        if (value.length <= 0) {
            alert("Please enter a value");
            ...
            $('#changeQuestionPerDayModal').modal('hide');
            var obj = {
                key: 'questionsPerDay',
                value: value
            };
            ...
            $("#changeQuestionPerDayInput").val("");
            postData(obj, "/changeSettings.php", success, error);
        }

    });

    $("#changeQuestionsPerWeekBtn").on("click", function() {
        $("#changeQuestionPerWeekModal").modal();
    });

    $("#changeQuestionPerWeekSubmit").on("click", function() {
        var value = $("#changeQuestionPerWeekInput").val();
        if (value.length <= 0) {
            alert("Please enter a value");
        } else if (!value.match(/^-?\d+$/)) {
            alert("Only numeric characters are allowed");
            ...
            $('#changeQuestionPerWeekModal').modal('hide');
            var obj = {
    ...
    $("#changeAutoTimeBtn").on("click", function() {
        adjustTimeModalOpen = true;
        $("#changeAutoTimesModal").modal();
    });

    $("#changeAutoTimeSubmit").on("click", function() {
        var value = $("#lastTime").val();
        // if (value.length <= 0) {
        //     alert("Please enter a value");
        // } else if (value.match(/[^0-9 ]+/)) {
        //     alert("Only numeric characters are allowed");
        ...
        //     alert("Value must be greater than 1");
        // } else {
            $('#changeAutoTimesModal').modal('hide');
            adjustTimeModalOpen = false;
            var obj = {
                key: 'lastTime',
                value: value.replace(":","")
            };
    ...
    $("#autoDisable").on("click", function() {
        var setting = $('#autoDisable').prop('checked');
        var obj = {
            key: 'autoDisable',
            value: setting
        };
    ...
    $("#resetDBBtn").on("click", function() {
        if (confirm("Are you sure you want to reset the database? This will delete everything! This can not be un-done!")) {
            ...
            var error = function() {
                console.log("error reseting the DB");
            };
            postData({
                confirmed: true
            }, "/DBInit.php", success, error);
        }
    });
...
function setUpEditRawData(event) {
    ...
    if (startDate.length > 0 && startDate !== "" && startTime.length > 0 && startTime !== "" && endTime.length > 0 && endTime !== "") {
        if (event.data.crunchNumbers) {
            ...
            if (startTime.getTime() > endTime.getTime()) {
                ...
                var success = function(data) {
                    ...
                    $("#editRawDataData").append("<div class='row'>" + "<div class='col-xs-1'><strong>ID</strong>" + "</div>" + "<div class='col-xs-1'>" + "<strong>Student net id</strong>" + "</div>" + "<div class='col-xs-1'>" + "<strong>Removed By</strong>" + "</div>" + "<div class='col-xs-2'>" + "<strong>Enqueue Time</strong>" + "</div>" + "<div class='col-xs-2'>" + "<strong>Dequeue Time</strong>" + "</div>" + "<div class='col-xs-1'>" + "<strong>Question</strong>" + "</div>" + "<div class='col-xs-1'>" + "<strong>Pass off flag</strong>" + "</div>" + "<div class='col-xs-2'>" + "<strong>Done Getting help time</strong>" + "</div>" + "<div class='col-xs-1'>" + "<strong>Action</strong>" + "</div>" + "</div>");

                    $.each(data.studentData, function(index, element) {
                        $("#editRawDataData").append("<div class='row' style='padding-bottom:5px;' id='" + element.id + "'>" + "<div class='col-xs-1'>" + element.id + "</div>" + "<div class='col-xs-1'>" + "<input type='text' id='editRawNetId' disabled='true' value='" + element.netId + "'/>" + "</div>" + "<div class='col-xs-1'>" + "<input type='text' id='editRawRemovedBy' disabled='true' value='" + element.removedBy + "'/>" + "</div>" + "<div class='col-xs-2'>" + "<input type='text' id='editRawEnqueueTime' disabled='true' value='" + element.enqueueTime + "'/>" + "</div>" + "<div class='col-xs-2'>" + "<input type='text' id='editRawDequeueTime' disabled='true' value='" + element.dequeueTime + "'/>" + "</div>" + "<div class='col-xs-1'>" + "<input type='text' id='editRawQuestion' disabled='true' value='" + element.question + "'/>" + "</div>" + "<div class='col-xs-1'>" + "<input type='text' id='editRawPassOff' disabled='true' value='" + element.passOff + "'/>" + "</div>" + "<div class='col-xs-2'>" + "<input type='text' id='editRawDoneGettingHelpTime' disabled='true' value='" + (element.doneGettingHelp == null ? "None" : element.doneGettingHelp) + "'/>" + "</div>" + "<div class='col-xs-1'>" + "<button class='btn btn-warning fa fa-edit' id='" + element.id + "Btn' onclick=editRawData('" + element.id + "')></button>" + /*"<button class='btn btn-danger fa fa-times' id='" + element.id + "Btn' onclick=editRawData('" + element.id + "', 'cancel')></button>" +*/
                        "</div>" + "</div>");
                    });
                }
...
function editRawData(id, action) {
    ...
    if (!inEditMode) {
        if (cancel) {//i don't know if i like this....
        } else {
...
function updateUI(data) {
    ...
    if (data.hasOwnProperty("settings")) {
        $.each(data.settings, function(index, obj) {
            $.each(obj, function(key, value) {
                if(key === "lastTime" && !adjustTimeModalOpen)
                {
                   $("#" + key).prop("value", value.slice(0,-2) + ":" + value.slice(-2));
                }
    ...
    if (data.hasOwnProperty("stats")) {
        ...
        $.each(data.stats.tas, function(index, obj) {
            ...
            var output = '<div class="row myRow" id="' + obj.netId + '"><div class="col-xs-2">' + (obj.active == 1 ? "TA" : "TA (non-active)") + '</div><div class="col-xs-3">' + obj.netId + '</div><div class="col-xs-3">' + obj.name + '</div><div class="col-xs-2">' + obj.helpCounter + '</div>' + '<div class="col-xs-2">' + obj.passOffCounter + '</div></div>';

            $("#statsList").append(output);

            //add each TA to the Edit TA page
            var editTAOutput = '<div class="row myRow" id="' + obj.netId + '"><div class="col-xs-3">' + obj.netId + '</div><div class="col-xs-3">' + obj.name + '</div><div class="col-xs-2">' + obj.helpCounter + '</div>' + '<div class="col-xs-2">' + obj.passOffCounter + '</div>' + '<div class="col-xs-2">' + '  <input id="' + obj.netId + 'ActiveToggle" type="checkbox" onchange="toggleTAActive(\'' + obj.netId + "'," + obj.active + ')"' + (obj.active == 1 ? 'checked' : '') + '></input>' + '  <button onclick="editTAName(\'' + obj.netId + '\')" class="btn btn-sm btn-warning fa fa-edit" style="margin-left:5px; margin-top:-5px;"></button>' + '</div>';
            /*
...
*/
        ...
        $.each(data.stats.students, function(index, obj) {
            ...
            var output = '<div class="row myRow" id="' + obj.netId + '"><div class="col-xs-2">Student</div><div class="col-xs-3">' + obj.netId + '</div><div class="col-xs-3">' + obj.name + '</div><div class="col-xs-2">' + obj.helpCounter + '</div>' + '<div class="col-xs-2">' + obj.passOffCounter + '</div></div>';
    ...
    if (data.hasOwnProperty("avgs")) {
        ...
        if (data.avgs.avg > 10)
            //just a random number above 0 to no give wrong results when noones on the queue
...
function stuChat() {
    showChatNote();
    //the alerts are annoying
    if (($("#chatModal").data('bs.modal') || {}).isShown)
        return;
    chat.clear();
    chat.enter(user);
    $('#chatModal').modal({
